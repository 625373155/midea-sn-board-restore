...
from protocol_reference import encode_sn, target_vectors, validate_body_sn
...
SUPPORTED_MODELS = ("KFR-26G/WXAA2@",)
EVIDENCE_HASH_VERSION = "nfkc-casefold-whitespace-v1"
TEMPLATE_DIRECTORY = Path(__file__).resolve().parent.parent / "assets" / "package-template"
PLUGIN_DIRECTORY = Path(__file__).resolve().parents[3]

# Public distributions must never embed a customer's device identity or repair
# history.  Device-specific history is append-only local state under
# MideaSnBoardRestore/generator/events.jsonl.  A private deployment may carry
# separately reviewed immutable events, but they must never be committed here.
...
def _validate_model(value: str) -> str:
    ...
    if value not in SUPPORTED_MODELS:
        raise GenerationError(
            "model is outside this public release's independently reviewed compatibility allowlist: "
...
def _validate_prior_evidence_hash_metadata(priors: Iterable[dict[str, Any]]) -> None:
    """Refuse evidence hashes whose normalization semantics cannot be trusted.

    Older events that never stored an evidence hash remain usable as conservative
    history records.  Once a record contains a hash, however, its normalization
    version and digest must match this generator release before the hash may
    participate in fresh-evidence checks.
    """

    for record in priors:
        if "newBoardEvidenceSha256" not in record:
            continue
        incident_id = str(record.get("incidentId", "(unknown)"))
        if record.get("newBoardEvidenceHashVersion") != EVIDENCE_HASH_VERSION:
            raise GenerationError(
                "prior event history contains a new-board evidence hash with a missing or unsupported "
                f"normalization version for incident {incident_id!r}; generation is refused until manual "
                "history migration and audit are completed"
            )
        evidence_hash = record.get("newBoardEvidenceSha256")
        if not isinstance(evidence_hash, str) or SHA256_RE.fullmatch(evidence_hash) is None:
            raise GenerationError(
                "prior event history contains a malformed new-board evidence hash for incident "
                f"{incident_id!r}; generation is refused until manual history migration and audit are completed"
            )
...
def generate(arguments: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        output_root.relative_to(PLUGIN_DIRECTORY)
    except ValueError:
        pass
    else:
        raise GenerationError(
            f"output may not be the plugin/skill/template tree or any of its descendants: {PLUGIN_DIRECTORY}"
        )
    ...
    with _ExclusiveHistoryLock(lock_path):
        ...
        if priors:
            _validate_prior_evidence_hash_metadata(priors)
            ...
            if any(
                record.get("newBoardEvidenceSha256") == new_board_evidence_sha256
                for record in priors
                if "newBoardEvidenceSha256" in record
            ):
                raise GenerationError(
        ...
        try:
            ...
            manifest: dict[str, Any] = {
                "schemaVersion": SCHEMA_VERSION,
                "generator": {
                    "name": "midea-sn-board-restore/new_restore_package.py",
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model", required=True, help="model read from this appliance/new-board evidence")
