...
EXPECTED_GENERATOR_NAME = "midea-sn-board-restore/new_restore_package.py"
...
def _validate_manifest(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if model not in SUPPORTED_MODELS:
        raise ValidationError("manifest model is outside this validator release's compatibility allowlist")
    previous_incident_id = repair.get("previousIncidentId")
    if previous_incident_id is not None and (
        not isinstance(previous_incident_id, str)
        or re.fullmatch(
...
def _validate_exact_template_tree(package_directory: Path, identity: dict[str, Any]) -> None:
    ...
    for template_path in template_paths:
        ...
        if target_path.read_bytes() != expected_bytes:
            raise ValidationError(
                f"runtime file is not the exact audited rendering from this plugin release: {rendered_name}"
            )
...
def _validate_powershell(package_directory: Path, identity: dict[str, Any]) -> Path:
    ...
    expected_send_calls = re.findall(
        r"(?im)^\s*Send-Packet\s+-Stream\s+\$(Stream|stream)\s+-Packet\s+\$(packet|diagnosticPacket|readPacket)\s*$",
        text,
    )
    if expected_send_calls != [
        ("Stream", "packet"),
        ("stream", "diagnosticPacket"),
        ("stream", "packet"),
        ("stream", "readPacket"),
    ]:
        raise ValidationError(f"packet-send call sites differ from the audited write/read sequence: {expected_send_calls}")
    if re.search(
